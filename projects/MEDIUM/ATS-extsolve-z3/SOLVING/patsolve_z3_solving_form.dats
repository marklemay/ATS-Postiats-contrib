...
implement formula_null() = formula_int(0)

implement formula_true() = 
  tt where
  {
    val (fpf | ctx) = the_Z3_context_vget()
...
implement formula_not(s2e1) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_not (ctx, s2e1)
    val () = Z3_dec_ref(ctx, s2e1)
    prval () = fpf(ctx)
  }

implement formula_disj (s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_or2 (ctx, s2e1, s2e2)
...
implement formula_conj(s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    
    val res = Z3_mk_and2 (ctx, s2e1, s2e2)
...
implement formula_impl(s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_implies (ctx, s2e1, s2e2)
...
implement formula_conj_list(s2es) = 
  let
    fun aux ( s2e0: form, s2es: formlst ) : form = 
      (
        case+ s2es of
          | ~list_vt_nil() => s2e0
          | ~list_vt_cons(s2e1, s2es2) => aux(formula_conj(s2e0, s2e1), s2es2)
      )
  in
    case+ s2es of
...
implement formula_conj_list1 (s2es_arg, s2e_res) = 
  let (* TODO: why is there a let here ?*)
  in
    case+ s2es_arg of
      | ~list_vt_nil() => s2e_res
      | list_vt_cons _ => formula_conj(formula_conj_list(s2es_arg), s2e_res)
  end
  
implement formula_impl_list1 (s2es_arg, s2e_res) = 
  let (* TODO: why is there a let here ?*)
  in
    case+ s2es_arg of
      | ~list_vt_nil() => s2e_res
      | list_vt_cons _ => formula_impl(formula_conj_list(s2es_arg), s2e_res)
  end

implement formula_ineg (s2e1) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_unary_minus (ctx, s2e1)
    val () = Z3_dec_ref(ctx, s2e1)
    prval () = fpf(ctx)
  }

implement formula_iadd(s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_add2 (ctx, s2e1, s2e2)
    val () = Z3_dec_ref(ctx, s2e1)
    val () = Z3_dec_ref(ctx, s2e2)
    prval () = fpf(ctx)
  }

implement formula_isub (s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_sub2 (ctx, s2e1, s2e2)
    val () = Z3_dec_ref(ctx, s2e1)
    val () = Z3_dec_ref(ctx, s2e2)
    prval () = fpf(ctx)
  }

implement formula_imul (s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_mul2 (ctx, s2e1, s2e2)
    val () = Z3_dec_ref(ctx, s2e1)
    val () = Z3_dec_ref(ctx, s2e2)
    prval () = fpf(ctx)
  }

implement formula_idiv(s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_div (ctx, s2e1, s2e2)
...
implement formula_ndiv (s2e1, s2e2) = formula_idiv(s2e1, s2e2)

implement formula_ilt (s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    
    val res =  Z3_mk_lt (ctx, s2e1, s2e2)
...
implement formula_ilte (s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_lte (ctx, s2e1, s2e2)
...
implement formula_igt (s2e1, s2e2) = 
  res where
  {
    val (fpf | ctx) = the_Z3_context_vget()
    val res = Z3_mk_gt (ctx, s2e1, s2e2)
...
val () = Z3_sort_dec_ref(ctx, res)
val () = Z3_sort_dec_ref(ctx, arg)
...
val () = Z3_dec_ref(ctx, res)
val () = Z3_dec_ref_arrayptr(ctx, args, n)
...
| S2Ecst(s2c) => let
  ...
  in
    ...
    | S2CINTnone() =>
      ...
      {
        val () = s2cfun_initize_s2cinterp(s2c)
      } (* [S2CINTnone] *)
