...
fun eq_stamp_stamp : (stamp, stamp) -<fun0> bool
fun neq_stamp_stamp : (stamp, stamp) -<fun0> bool
fun compare_stamp_stamp : (stamp, stamp) -<fun0> int
...
fun eq_symbol_symbol : (symbol, symbol) -<fun0> bool
fun neq_symbol_symbol : (symbol, symbol) -<fun0> bool
fun compare_symbol_symbol : (symbol, symbol) -<fun0> int
...
datatype s2rt =
  ...
  | S2RTfun of (s2rtlst, s2rt) (*args, result*)
...
fun s2rt_real(): s2rt
fun s2rt_string(): s2rt

fun s2rt_is_impred(s2t: s2rt): bool
...
fun s2cst_make (symbol, s2rt, stamp): s2cst
...
fun s2rtdat_get_sconlst (s2rtdat): s2cstlst
fun s2rtdat_make(symbol, stamp, s2cstlst): s2rtdat
...
fun s2var_make (symbol, s2rt, stamp): s2var
...
fun print_tyreckind : tyreckind -> void
and prerr_tyreckind : tyreckind -> void (* AND??????*)
...
datatype s2exp_node =
  ...
  | S2Emetdec of (s2explst, s2explst) // met,bound ... strictly decreasing
  
  | S2Etop of (int, s2exp) (* int:kind *)
  
  | S2Einvar of (s2exp)
  
  | S2Efun of (int, s2explst, s2exp) (*int ? npf*)
  
  | S2Euni of (s2varlst, s2explst, s2exp) (* s2explst s2ps, s2exp scope*)
  | S2Eexi of (s2varlst, s2explst, s2exp)
  
  | S2Etyrec of (tyreckind, int, labs2explst)
...
fun s2exp_make_node (s2t: s2rt, node: s2exp_node): s2exp
...
datatype c3nstrkind =
  ...
  | C3TKsome_fin of (d2var, s2exp, s2exp) (*s2exp fin*)
  | C3TKsome_lvar of (d2var, s2exp, s2exp) (*s2exp lvar*)
  | C3TKsome_vbox of (d2var, s2exp, s2exp) (*s2exp vbox*)
...
fun h3ypo_make_node (loc_t, h3ypo_node): h3ypo

fun c3nstr_make_node (loc: loc_t, knd: c3nstrkind, c3nstr_node): c3nstr
